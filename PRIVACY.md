...
The local autonomous trading profile persists a small account-specific ledger under `.xrocket-mcp` (or `XROCKET_AGENT_STATE_PATH`). It contains generated client order identifiers, symbols, UTC dates, policy-valued USD amounts, and reconciliation status; it does not contain the bearer token, balances, order payloads, or account responses. Prepared transfer/withdrawal intents and approval receipts remain process-memory-only. Your MCP client, terminal, operating system, deployment platform, or xRocket may keep separate logs or records under their own policies; configure those systems accordingly.
