...
- Gemini CLI extension: the root `gemini-extension.json` connects to the hosted public-only endpoint and can be installed directly from the repository. The `gemini-cli-extension` repository topic enables automatic daily Gallery discovery without a separate submission form.
...
| P1 | Gemini CLI Gallery | Root `gemini-extension.json` plus GitHub topic `gemini-cli-extension` | **Direct-install extension validated; discovery topic enabled** | Verify automatic Gallery ingestion after the next daily crawl |
