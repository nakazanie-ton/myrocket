...
describe("hosted Streamable HTTP server", () => {
  ...
  it("serves health and the exact public MCP catalog over Streamable HTTP", async () => {
    ...
    expect(client.getInstructions()).toContain("autonomous trading profile");
  ...
  it("serves a secure zero-install landing page and fixed Open xRocket redirect", async () => {
    ...
    expect(landing.body).toContain("Give your agent");
    expect(landing.body).toContain("trading-config --limit 100 --asset USD");
    expect(landing.body).toContain("Menu → Settings → Exchange settings → API token");
    expect(landing.body).toContain("Orders inside the limit execute without asking again");
    expect(landing.body).toContain("hosted MCP endpoint never receive account credentials");
    expect(landing.body).toContain("Transfers</span><b>EXPLICIT ONLY");
    expect(landing.body).not.toContain("Review every order");
