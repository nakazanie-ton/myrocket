...
`GET /health` is used by the `xrocket-mcp doctor` CLI rather than exposed as an agent semantic tool. The audited seven WebSocket channels are not exposed in 0.5.0; use bounded REST snapshots. xRocket Pay endpoints are a separate product and are unavailable.
